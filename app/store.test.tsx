...
jest.mock('chess.js', () => {
  return {
    Chess: jest.fn().mockImplementation((fen) => {
      return {
        fen: jest.fn().mockReturnValue(fen || 'rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1'),
...
describe('Game Store', () => {
  ...
  beforeEach(() => {
    ...
    // Create a mock store with controlled get/set functions
    set = jest.fn();
...
describe('useStoreSync', () => {
  ...
  test('fetches and syncs store on initial render', async () => {
    // Render the hook
    const { result } = renderHook(() => useStoreSync('test-id', store));
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/game/test-id?name=game-test-id');
  ...
  test('polls for updates at regular intervals', async () => {
    // Render the hook
    renderHook(() => useStoreSync('test-id', store));
  ...
  test('cleans up interval on unmount', () => {
    ...
    // Render the hook
    const { unmount } = renderHook(() => useStoreSync('test-id', store));
