...
describe("Game API Routes", () => {
  ...
  describe("GET /api/game/[id]", () => {
    test("returns game state when found", async () => {
      ...
      const request = createMockRequest(
        "http://localhost:3000/api/game/1?name=test-state",
        "GET",
      );
    ...
    test("returns 404 when game is not found", async () => {
      ...
      // Create mock request
      const request = createMockRequest(
        "http://localhost:3000/api/game/999?name=test-state",
        "GET",
      );
    ...
    test("returns 400 when name parameter is missing", async () => {
      ...
      const request = createMockRequest(
        "http://localhost:3000/api/game/1",
        "GET",
      );
    ...
    test("returns 500 when an error occurs", async () => {
      // Mock database error
      (db.query.gamesTable.findFirst as jest.Mock).mockRejectedValue(
  ...
  describe("POST /api/game/[id]", () => {
    test("updates game state successfully", async () => {
      ...
      const requestBody = {
        name: "test-state",
        state: JSON.stringify({ value: "updated" }),
      };
      const request = createMockRequest(
        "http://localhost:3000/api/game/1",
        "POST",
    ...
    test("returns 404 when game is not found", async () => {
      ...
      const requestBody = {
        name: "test-state",
        state: JSON.stringify({ value: "test" }),
      };
      const request = createMockRequest(
        "http://localhost:3000/api/game/999",
        "POST",
        requestBody,
      );
  ...
  describe("DELETE /api/game/[id]", () => {
    ...
    test("handles case when state key does not exist", async () => {
      ...
      // Create mock request
      const request = createMockRequest(
        "http://localhost:3000/api/game/1?name=non-existent-key",
        "DELETE",
      );
